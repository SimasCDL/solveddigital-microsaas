...
type State =
  "idle" | "sorting" | "sorted" | "generating" | "stitching" | "done" | "error";
...
export default function Generate() {
  ...
  const getAdminKey = () => {
    const key =
      localStorage.getItem("admin_key") || window.prompt("Access key:") || "";
  ...
  const fetchListing = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(body?.error || "Could not fetch that listing");
      ...
      setError(
        err instanceof Error ? err.message : "Could not fetch that listing",
      );
    } finally {
      setIngesting(false);
    }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const legacy: HistoryEntry[] = JSON.parse(
          localStorage.getItem("walkthrough_history") || "[]",
        );
        if (legacy.length) {
          await Promise.all(
            legacy.map((e) =>
  ...
  const saveToHistory = async (
    ...
    const thumbnails = await Promise.all(
      photos.map((p) =>
        p.file ? toThumbnail(p.file) : Promise.resolve(p.url || ""),
      ),
    );
  ...
  const sortPhotos = async () => {
    ...
    try {
      ...
      const sortedPhotos: SortedPhoto[] = order.map(
        (o: { index: number; label: string }) =>
          remoteUrls.length
  ...
  const generate = async () => {
    ...
    const clipSlots: string[] = new Array(
      mode === "walkthrough" ? photos.length * 2 - 1 : photos.length,
    );
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (
            (event.type === "progress" || event.type === "clip_ready") &&
            event.total
          )
            setClipTotal(event.total);
          ...
          if (event.type === "done") {
            ...
            const orderedClips: string[] = event.videoUrls?.length
              ? event.videoUrls
              : (clipSlots.filter(Boolean) as string[]);
            if (!orderedClips.length)
              throw new Error("No clips were generated. Please try again.");
  ...
  const expectedClips =
    clipTotal ||
    (mode === "walkthrough" ? Math.max(photoCount * 2 - 1, 0) : photoCount);
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <main
        style={{ maxWidth: 860, margin: "0 auto", padding: "48px 20px 80px" }}
      >
        <div
          style={{
            display: "grid",
        ...
        >
          <div>
            ...
            >
              Drop your listing photos. AI sorts them, then generates cinematic
              1080p clips and stitches them into one video.
            </p>
            ...
            {(state === "idle" || state === "sorted") && (
              <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                ...
                <button
                  ...
                  disabled={ingesting || !listingUrl.trim()}
                >
              ...
              remoteUrls.length > 0 && (
                ...
                >
                  <p className="cap" style={{ marginBottom: 8 }}>
                    {remoteUrls.length} listing photo
                    {remoteUrls.length > 1 ? "s" : ""}
                  </p>
                  <div
                    style={{
                      display: "grid",
              ...
              remoteUrls.length === 0 && (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 12,
                    padding: files.length ? 12 : "32px 16px",
                    textAlign: "center",
                    cursor: "pointer",
                    background: "var(--bg2)",
                    marginBottom: 12,
                  ...
                  {sorted.map((s, i) => (
                    ...
                    >
                      <div style={{ aspectRatio: "1", position: "relative" }}>
                        <img
                          src={s.file ? URL.createObjectURL(s.file) : s.url}
                          alt=""
                          style={{
                            width: "100%",
                            height: "100%",
            ...
            {state === "generating" && expectedClips > 0 && (
              <div style={{ marginBottom: 12 }}>
                <p className="cap" style={{ marginBottom: 6 }}>
                  Clips done: {clipsDone}/{expectedClips}
                </p>
                  ...
                  {Array.from({ length: expectedClips }).map((_, i) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 99,
                        background:
                          i < clipsDone ? "var(--ink)" : "var(--border)",
            ...
            {state === "idle" && (
              <button
                ...
                onClick={sortPhotos}
                disabled={!files.length && !remoteUrls.length}
                style={{ width: "100%" }}
              >
                {photoCount > 0
                  ? `Analyse & sort ${photoCount} photo${photoCount > 1 ? "s" : ""}`
                  : "Analyse & sort photos"}
              </button>
            )}
            {state === "sorting" && (
                  ...
                  >
                    ...
                    {entry.videoUrl ? (
                      ...
                      <button
                        ...
                        onClick={() => restitchEntry(entry)}
                        disabled={restitching === entry.id}
                      >
                        {restitching === entry.id
                          ? "Merging..."
                          : "Re-stitch video"}
                      </button>
                    )}
