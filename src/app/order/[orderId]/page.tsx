      ...
      <main className="mx-auto w-full max-w-3xl px-4 pb-24 pt-12 sm:px-6 sm:pt-16">
        {error ? (
          <div className="text-center">
            <h1 className="font-display text-3xl text-tink">Something went wrong</h1>
          ...
          <div className="text-center">
            <h1 className="font-display text-3xl text-tink">This link has expired</h1>
            <p className="mx-auto mt-3 max-w-md text-tink-soft">
              Video links are available for 7 days after delivery. If you still
              need your tour, reply to your delivery email and we&apos;ll help you out.
            </p>
            <p className="mt-6 text-[13px] text-tink-soft/80">Order #{orderId}</p>
          </div>
          ...
          <div>
            <div className="mb-8 text-center">
              ...
              <h1 className="font-display mt-5 text-4xl text-tink sm:text-5xl">Your tour is ready</h1>
              {data.propertyAddress && <p className="mt-3 text-tink-soft">{data.propertyAddress}</p>}
            </div>
            ...
            <div className="rounded-3xl border border-line bg-paper p-4 shadow-xl shadow-black/5 sm:p-6">
              ...
              {data.videoUrls.length > 1 && (
                <div className="mt-8">
                  <p className="eyebrow mb-3 text-tink-soft">Vertical versions — for Reels & TikTok</p>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {[
                      { label: 'Vertical (9:16)', hint: 'Full frame', file: 'tourly-vertical.mp4' },
                      { label: 'Vertical full-screen', hint: 'Fills the screen', file: 'tourly-vertical-fullscreen.mp4' },
                    ].map((f, i) =>
                      data.videoUrls[i + 1] ? (
                        <a
                          key={i}
                        ...
                        >
                          <span className="text-sm font-medium text-tink">{f.label}</span>
                          <span className="text-[13px] text-tink-soft">{f.hint}</span>
                        </a>
                      ) : null
                    )}
                  </div>
          ...
          <div className="text-center">
            <h1 className="font-display text-3xl text-tink">Generation failed</h1>
            <p className="mx-auto mt-3 max-w-md text-tink-soft">
              We&apos;ve been notified and we&apos;ll make it right — check your email for details.
            </p>
          </div>
        ) : (
          <div className="text-center">
            ...
            <h1 className="font-display text-3xl text-tink sm:text-4xl">Creating your tour</h1>
            ...
            <p className="mt-8 text-[13px] text-tink-soft/80">
              This page updates automatically · We&apos;ll also email you when it&apos;s done
            </p>
          </div>
        )}
      </main>
