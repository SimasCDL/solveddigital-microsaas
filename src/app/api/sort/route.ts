...
import { classifyPhoto, planWalkthrough, ROOM_ORDER, type PhotoSource } from '@/lib/sort';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if ((req.headers.get('content-type') || '').includes('application/json')) {
      ...
      try {
        ...
        return NextResponse.json({
          sorted: plan.order.map(i => ({ index: i, label: plan.labels[i] ?? 'other' })),
        });
      } catch (err) {
        console.error('[sort] plan failed, falling back to original order:', err);
        return NextResponse.json({
    ...
    try {
      ...
      console.error('[sort] plan failed, falling back to per-photo labels:', err);
    }

    const classifications = await Promise.all(
      sources.map(async (s, i) => {
        const label = 'base64' in s ? await classifyPhoto(s.base64, s.mediaType) : 'other';
